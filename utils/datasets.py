    ...
    def __getitem__(self, item):
        ...
        if self.condition.strip() in ["train", "Train", "TRAIN"]:
            ...
            if random.random() < 0.5:
                img_A = Image.fromarray(self._clahe_process(np.array(img_A)))
            # img_B = Image.fromarray(self._clahe_process(np.array(img_B)))
            ### CLAHE ###
        ...
        return {"A": img_A, "B": img_B, "label": self.labels[item]}
