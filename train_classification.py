...
if __name__ == '__main__':
    ...
    csv_file_path = opt.dataset_path + "/" + "list_classification_20190821.csv"
    ...
    save_ckpt_name = r"%s-%s-%s-(%s)-ep(%d)-bs(%d)-lr(%s)-img_size(%d, %d, %d)-crop_size(%d, %d, %d)-val_index(%d)" \
                     % (exp_name, today, dataset_name, model_name, opt.n_epochs, opt.batch_size, opt.lr, opt.img_height,
                        opt.img_width,
                        opt.channels, opt.img_crop_height, opt.img_crop_width, opt.channels, val_idx)
    ...
    train_classification_compiler = GlaucomaClassificationCompiler(**training_configuration)
    train_classification_compiler.train()
